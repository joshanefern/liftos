...
const WorkoutItem = ({ name, type, duration, calories, time, index }: WorkoutItemProps) => (
  ...
  >
    <div className="flex items-center gap-4">
      <div className="w-9 h-9 rounded-lg surface-3 flex items-center justify-center">
        <Dumbbell size={15} className="text-gold" />
      </div>
      <div>
        <p className="text-sm font-medium">{name}</p>
        <p className="text-xs text-[hsl(var(--text-tertiary))]">{type}</p>
      </div>
    </div>
    <div className="flex items-center gap-5">
      ...
      <span className="text-xs text-[hsl(var(--text-tertiary))] w-16 text-right">{time}</span>
    </div>
  </div>
);
